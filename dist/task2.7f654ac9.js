...
if (databaseJSON == null) localStorage.setItem('database', JSON.stringify({
    database: []
}));
function clearList() {
...
function render() {
    ...
    for (let item1 of deleteButtons)item1.addEventListener('click', function() {
        ...
        let databaseJSON1 = JSON.parse(localStorage.getItem('database'));
        let deleteIndex = databaseJSON1.database.indexOf(textLink);
        console.log(databaseJSON1.database);
        databaseJSON1.database.splice(deleteIndex, 1);
        console.log(databaseJSON1.database);
        console.log(databaseJSON1);
        localStorage.setItem('database', JSON.stringify(databaseJSON1));
        buttLi.remove();
    });
}
render();
addBut.addEventListener('click', function() {
    ...
    if (addInputText == '' || addInput2Text == '') alert('You must write text');
    else {
        databaseJSON.database.push([
            addInputText,
            addInput2Text
        ]);
